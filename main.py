...
def extract_reader_text(url):
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if downloaded:
                main_content = trafilatura.extract(downloaded, include_comments=False, include_tables=False)
                return main_content or ''
            else:
                print(f'Failed to fetch content for {url}')
                return ''
        elif response.status_code == 403:
            print(f'HTTP 403 Forbidden for {url}. Retrying with cookies...')
            # Try again with cookies (simulate a browser session)
            session = requests.Session()
            session.headers.update(headers)
            session.get(url)  # Get cookies
            ...
            if response.status_code == 200:
                ...
                if downloaded:
                    main_content = trafilatura.extract(downloaded, include_comments=False, include_tables=False)
                    return main_content or ''
            print(f'Failed to bypass 403 for {url}')
            return ''
        else:
...
def print_heading():
    heading = r'''
 __  __ _____ _     _____ _____  ____  ____   ___  ____  ____  
|  \/  | ____| |   | ____|_   _|/ ___||  _ \ / _ \|  _ \|  _ \ 
| |\/| |  _| | |   |  _|   | |  \___ \| |_) | | | | |_) | | | |
| |  | | |___| |___| |___  | |   ___) |  _ <| |_| |  _ <| |_| |
|_|  |_|_____|_____|_____| |_|  |____/|_| \_\\___/|_| \_\____/ 
'''
    print(Fore.GREEN + heading + Style.RESET_ALL)
    print(Fore.GREEN + 'Welcome to HTML TO PDF - Old School Terminal Edition' + Style.RESET_ALL)
...
def main():
    while True:
        ...
        if choice == '1':
            ...
            full_html = '''<!DOCTYPE html>\n<html>\n<head>\n  <meta charset="utf-8">\n  <style>body { font-family: Arial, monospace; color: #00FF00; background: #000; } h1, h2, a { color: #00FF00; } .site-section { margin-bottom: 60px; } .site-link { font-size: 0.95em; margin-bottom: 1em; display: block; } .page-break { page-break-before: always; } hr { border: none; border-top: 1px solid #00FF00; margin: 2em 0; }</style>\n</head>\n<body>\n  <h1>Combined Website Stories</h1>\n'''
            full_html += '\n'.join(sections)
            full_html += '\n</body>\n</html>'
            with tempfile.NamedTemporaryFile(delete=False, suffix='.html') as tmp:
                tmp.write(full_html.encode('utf-8'))
